...
public class Main {
    static int M, N, zerocnt; //zerocnt : 맵 전체의 1개수 파악용 변수
    static int[][] map;
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < M; j++) {
                ...
                map[i][j] = idx;
                if (idx == 1) queue.offer(new Point(i, j)); //익은 토마토의 위치 큐 추가
        ...
        while (true) { //탐색시작
            ...
            for (int t = 0; t < size; t++) { //첫 탐색의 큐 사이즈만큼만 반복
                ...
                for (int i = 0; i < 4; i++) { //4방탐색 먼저
                    ...
                    if (map[nx][ny] == 0) { //덜 익은 토마토일 경우
                        map[nx][ny] = 1; //값 갱신
                        zerocnt--; //맵 전체의 0개수 1감소
                        queue.add(new Point(nx, ny)); //방금 익은 토마토의 위치 큐 추가
                    }
                }
            }
